...
describe('EmailConfirmationService', () => {
	...
	it('should not send a confirmation email to an already confirmed user', async () => {
		...
		await expect(emailConfirmationService.send(user)).rejects.toThrowError(EmailAlreadyConfirmedException);
	});
	...
	it('should confirm a user', async () => {
		...
		usersService.updateRole = jest.fn().mockResolvedValue(updatedUser);
	...
	it('should not confirm an unknown user', async () => {
		...
		const payload: EmailConfirmationTokenPayloadType = { email: 'janeth@datadvisor.me' };
